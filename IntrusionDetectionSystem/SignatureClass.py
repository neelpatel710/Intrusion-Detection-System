    ...
    def DistrubutedDOS(self, flag, threshold, time_in_sec):
        ...
        if len(Signatures.flood_type[flag].keys()) > 1:
            ...
            if total_sum >= threshold and min_time <= (datetime.datetime.now() + time_diff):
                ...
                self.dos_attack = ["Distributed Denial of Service", self.src_ip, flag]

    def FTPBruteForce(self, threshold, time_in_sec):
        time_diff = datetime.timedelta(seconds=time_in_sec)
        if self.src_ip not in Signatures.ftp_login.keys():
            Signatures.ftp_login[self.src_ip] = [1, datetime.datetime.now()]
        else:
            Signatures.ftp_login[self.src_ip][0] = Signatures.ftp_login[self.src_ip][0] + 1
            if Signatures.ftp_login[self.src_ip][0] >= threshold and Signatures.ftp_login[self.src_ip][1] <= (datetime.datetime.now() + time_diff):
                print("Alert! FTP Brute Force Login detected from IP: {}!".format(self.src_ip))
                self.dos_attack = ["FTP Brute Force Attack", self.src_ip]
            elif Signatures.ftp_login[self.src_ip][0] < threshold and (datetime.datetime.now() > Signatures.ftp_login[self.src_ip][1] + time_diff):
                Signatures.ftp_login.pop(self.src_ip)
